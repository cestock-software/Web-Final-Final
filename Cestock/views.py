...
def get_atencion_detalle(request):
    if request.POST:
        atencion = Atencion_Medica.objects.filter(
            id=request.POST['id']).first()
        detalle_atencion = Detalle_Atencion.objects.filter(
            atencion_medica=atencion).first()
        ficha = Carnet_Paciente.objects.filter(
            nro_ficha=int(str(atencion.nro_ficha))).first()
        date = atencion.fecha_atencion_medica.strftime('%d/%m/%Y')
        nombre_paciente = ficha.rut_paciente.nombre + ' ' + \
            ficha.rut_paciente.ap_paterno + ' ' + ficha.rut_paciente.ap_materno
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        userobj = UserSistema.objects.filter(
            rut=rut, tipo_usuario="medico", estado=True).first()
        if userobj:
            user = auth.authenticate(
                username=userobj.username, password=password)
        ...
        if user:
            ...
            userobj = UserSistema.objects.filter(
                rut=rut, tipo_usuario="administrador").first()
            if userobj:
                user = auth.authenticate(
                    username=userobj.username, password=password)
            form = LoginForm(user, request.POST)
            if user:
                ...
                messages.add_message(
                    request, messages.ERROR, 'Rut de usuario o contraseña incorrecta', extra_tags='inicio de sesión')
    else:
        form = LoginForm()
...
def PaginaPrincipal(request):
    ...
    primer_dia_semana = dia_actual - \
        datetime.timedelta(days=numero_dia_en_semana)
    ...
    while primer_dia_semana <= ultimo_dia_semana:
        if request.user.is_staff:
            atenciones_medicas_dia = Atencion_Medica.objects.filter(
                fecha_atencion_medica__day=primer_dia_semana.day, fecha_atencion_medica__month=primer_dia_semana.month, fecha_atencion_medica__year=primer_dia_semana.year)
        else:
            atenciones_medicas_dia = Atencion_Medica.objects.filter(nombre_medico=request.user.username, fecha_atencion_medica__day=primer_dia_semana.day,
                                                                    fecha_atencion_medica__month=primer_dia_semana.month, fecha_atencion_medica__year=primer_dia_semana.year)
        ...
        primer_dia_semana += d
    if request.user.is_staff:
        ultimas_atenciones_medicas = Atencion_Medica.objects.order_by(
            '-id')[:4]
    else:
        ultimas_atenciones_medicas = Atencion_Medica.objects.filter(
            nombre_medico=request.user.username).order_by('-id')[:4]
    context = {
...
def crearAtencionMedica(request):
    if request.method == "POST":
        ...
        if form.is_valid() and formP.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, 'se ha creado con exito', extra_tags='Atencion Medica')
            presc = formP.save()
            # presc.save()
            # print(presc)
            # if atencionmedica:
